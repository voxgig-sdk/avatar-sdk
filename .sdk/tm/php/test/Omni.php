...
//    only - a typed AvatarContext there would make every ctx
...
final class AvatarOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AvatarContext($clean, null);
    ...
    public static function err_from_map($m): ?AvatarError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AvatarError(is_string($code) ? $code : '', $msg);
    }
