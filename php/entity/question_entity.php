...
class QuestionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new QuestionEntity($this->_client, $opts);
    }

    /**
     * @param Question|array $args Question data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AvatarHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Question|array The current Question data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Question fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AvatarHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Question fields).
     */
     ...
     *
     * @param QuestionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed QuestionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Question|array The loaded Question as an assoc-array at the
     *   SDK boundary; throws AvatarError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AvatarHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Question items matching the given filter.
     *
     * @param QuestionListMatch|array|null $reqmatch Match filter (any subset
     *   of Question fields) as an assoc-array; QuestionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Question[]|array A list of Question items as assoc-arrays at
     *   the SDK boundary; throws AvatarError on failure (item-5 convention).
     */
