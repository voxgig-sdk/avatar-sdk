...
class FtCtrl extends AvatarControl
{
...
class FtClient
{
    ...
    private AvatarUtility $utility;

    public function __construct(AvatarUtility $utility, array $options)
    {
    ...
    public function get_utility(): AvatarUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AvatarContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AvatarContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AvatarConfig::make_feature($name);
    ...
    private static function buildUrl(AvatarSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AvatarError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AvatarResponse($fetched);
        }
    ...
    private function fail(AvatarContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AvatarContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
