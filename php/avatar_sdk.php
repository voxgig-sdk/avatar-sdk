...
class AvatarSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AvatarHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AvatarHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AvatarFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AvatarFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AvatarUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AvatarHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AvatarHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AvatarHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AvatarHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AvatarError("graphql_error",
                "AvatarSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Question($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_question === null) {
                $this->_question = new QuestionEntity($this, null);
            }
            return $this->_question;
        }
        return new QuestionEntity($this, $data);
    }
