// Command Avatar-cli is a CLI + REPL for the Avatar SDK,
...
const entitiesHelp = "character episode question"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AVATAR_APIKEY (api key), AVATAR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AVATAR_APIKEY carries the API key and
	// AVATAR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AVATAR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AVATAR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAvatarSDK(opts)
