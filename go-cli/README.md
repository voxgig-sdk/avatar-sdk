...
boru-driven command-line client **and** interactive REPL for the Avatar
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/avatar-cli)
make build
...
./avatar-cli list character
./avatar-cli load 1 character            # {id:1} shorthand
./avatar-cli load '{id:1}' character       # explicit match map
./avatar-cli list episode

# 5. Override the API base URL for a single call
AVATAR_BASE=https://api.example.com ./avatar-cli list character

# 6. No arguments -> interactive REPL
./avatar-cli
avatar> list character
avatar> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/avatar-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/avatar-cli list character
   ```
...
```sh
./avatar-cli load 1 character          # scalar shorthand for {id:1}
./avatar-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `avatar>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/avatar-cli`. |
...
(`./avatar-cli <expr>`) and interactively (the REPL), and expressions compose the
