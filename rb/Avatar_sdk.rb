...
class AvatarSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AvatarHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AvatarHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AvatarFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AvatarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AvatarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AvatarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    allow_op = VoxgigStruct.getpath(@options, "allow.op")
    {
      ...
      "err" => AvatarError.new(
        "#{op}_allow",
        "AvatarSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = AvatarHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AvatarError.new(
        "graphql_error", "AvatarSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Question.list / client.Question.load({ "id" => ... })
  def Question(data = nil)
    require_relative 'entity/question_entity'
    QuestionEntity.new(self, data)
  end
