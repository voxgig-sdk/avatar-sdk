...
class QuestionEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = AvatarSDK.test(seed, nil)
    seen = base.Question(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = AvatarConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = AvatarSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.Question(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = question_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "question." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AVATAR_TEST_QUESTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    question_ref01_data = nil
    if question_ref01_data_raw.length > 0
...
def question_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "question", "QuestionTestData.json")
  ...
  entid_env_raw = ENV["AVATAR_TEST_QUESTION_ENTID"]
  ...
  if env["AVATAR_TEST_LIVE"] == "TRUE"
    ...
    client = AvatarSDK.new(Helpers.to_map(merged_opts))
  end
