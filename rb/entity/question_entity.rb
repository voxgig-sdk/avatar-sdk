...
class QuestionEntity
  ...
  def make
    opts = @_entopts.dup
    QuestionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AvatarHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Question, Hash] the current Question data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AvatarHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Question fields)
  def match_get
  ...
  #
  # @param reqmatch [QuestionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Question.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Question, Hash] the loaded Question; raises AvatarError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AvatarHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [QuestionListMatch, Hash, nil] match filter (any subset of
  #   Question fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Question>, Array] the matching Question items; raises AvatarError on failure
