...
The Lua SDK for the Avatar API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/avatar-sdk/releases)),
...
Create an instance: `local question = client:Question(nil)`
...
```lua
local question, err = client:Question():load({ id = "question_id" })
```

#### Example: List

```lua
local questions, err = client:Question():list()
```
...
```
lua/
├── avatar_sdk.lua    -- Main SDK module
...
The main module (`avatar_sdk`) exports the SDK constructor
