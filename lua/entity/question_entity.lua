...
local QuestionEntity = {}
QuestionEntity.__index = QuestionEntity


function QuestionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, QuestionEntity)
  self._name = "question"
  self._client = client
...
function QuestionEntity:get_name()
  return self._name
end


function QuestionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return QuestionEntity.new(self._client, opts)
end
...
function QuestionEntity:mark_deleted()
  self._deleted = true
end


function QuestionEntity:deleted()
  return true == self._deleted
end


function QuestionEntity:data_set(args)
  if args ~= nil then
...
function QuestionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function QuestionEntity:match_set(args)
  if args ~= nil then
...
function QuestionEntity:match_get()
...
function QuestionEntity:stream(action, args, callopts)
...
function QuestionEntity:_run_op(ctx, post_done)
  local utility = self._utility
