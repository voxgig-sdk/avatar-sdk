...
//     (\`new AvatarSDK(...)\` / \`AvatarSDK.test(...)\` become
//     \`AvatarSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"question":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AvatarSDK(...)\` and any
// \`AvatarSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AvatarSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AvatarSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
