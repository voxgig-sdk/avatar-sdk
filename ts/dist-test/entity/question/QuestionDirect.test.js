...
(0, node_test_1.describe)('QuestionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-question', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-question', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-question', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-question', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.AVATAR_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.AvatarSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['AVATAR_TEST_QUESTION_ENTID'];
    ...
    const client = new __1.AvatarSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=QuestionDirect.test.js.map