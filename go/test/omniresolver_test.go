...
import (
	...
	omni "github.com/voxgig-sdk/avatar-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AvatarError); is {
				out := map[string]any{
					"name":    "AvatarError",
...
func omniCtx(arg any, client *sdk.AvatarSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AvatarSDK); is {
			client = live
