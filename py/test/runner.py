...
class AvatarTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AvatarTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AvatarTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AvatarTestRunner.getenv("AVATAR_TEST_LIVE")
        override = AvatarTestRunner.getenv("AVATAR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AvatarTestRunner.getenv(key)
        ...
        explain = AvatarTestRunner.getenv("AVATAR_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["AVATAR_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AvatarTestRunner._test_control is not None:
            return AvatarTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AvatarTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AvatarTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AvatarTestRunner.load_test_control()
...
def load_env_local():
    AvatarTestRunner.load_env_local()


def env_override(m):
    return AvatarTestRunner.env_override(m)


def entity_data(v):
    return AvatarTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AvatarTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AvatarTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AvatarTestRunner.load_test_control()


def live_client_options():
    return AvatarTestRunner.live_client_options()


def live_delay_ms():
    return AvatarTestRunner.live_delay_ms()
