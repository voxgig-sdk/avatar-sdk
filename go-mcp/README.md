...
[MCP](https://modelcontextprotocol.io) server exposing the Avatar SDK as
two agent tools — `avatar_list` and `avatar_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/avatar-mcp)
make build

# 2. Provide credentials via the environment
export AVATAR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user avatar \
  -- /absolute/path/to/avatar-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./avatar-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// avatar_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// avatar_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/avatar-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user avatar \
     -- "$PWD"/dist/*/avatar-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `avatar_list` and `avatar_load` tools now appear
   in new sessions. Ask the agent to *"list character using avatar"*
   and it calls `avatar_list` with `{"entity":"character"}`.
...
```sh
export AVATAR_APIKEY=sk_live_xxx            # API key
export AVATAR_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `avatar_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `avatar_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AVATAR_APIKEY` | API key sent with every request. |
| `AVATAR_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

character | episode | question

### Smoke test via HTTP (raw JSON-RPC)

```sh
./avatar-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"avatar_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
