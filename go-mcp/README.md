...
MCP server exposing the Avatar SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `avatar_list` | `entity`, optional `query` map | First page of records as JSON |
| `avatar_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `avatar_list` and
`avatar_load` tools become available in new sessions.
